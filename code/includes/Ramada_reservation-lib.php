...
class FM_CaptchaValidator extends FM_CustomValidator
{
   ...
   private $input_field;
   private $formid;
   private $error_on_empty_code;
   private $error_on_code_mismatch;
   private $logger;
   private $globaldata;
   private $clear_session;
   private $case_insensitive_match;
   ...
   function Init(&$logger,&$globaldata)
   {
      $this->logger = &$logger;
   ...
   function DoValidate(&$formars,&$error_hash)
   {
      ...
      else
      {
         ...
         $orig_pwd = $this->globaldata->session[$this->session_var];
         $orig_pwd2 ='';

        $key_input_name = $this->input_field.'_k';
        $captcha_session_key = $_GET[$key_input_name];
        if(empty($captcha_session_key))
        {
            $captcha_session_key = $formars[$key_input_name];
        }
        
        $this->logger->LogInfo("Captcha:captcha_session_key $captcha_session_key");
        $this->logger->LogInfo("Captcha:sfm_captcha_sessions:".@print_r($this->globaldata->session["sfm_captcha_sessions"],true));
        
        if(!empty($captcha_session_key) &&
           !empty($this->globaldata->session["sfm_captcha_sessions"]) &&
           !empty($this->globaldata->session["sfm_captcha_sessions"][$captcha_session_key]))
        {
            $orig_pwd2 = $this->globaldata->session["sfm_captcha_sessions"][$captcha_session_key];
        }
            
         $this->logger->LogInfo("Captcha: orig_pwd $orig_pwd orig_pwd2 $orig_pwd2 input_pwd $input_pwd");

         if(strcmp($input_pwd,$orig_pwd) !=0 )
         {
            if(strcmp($input_pwd,$orig_pwd2) != 0)
            {
         ...
         {
             $this->globaldata->session[$this->session_var] = '';
         }
      }
      return $ret;
   }
...
{  
   ...
   private $linecolor;
   private $code;
   private $show_captcha_var;
   private $fm_validator;
   private $captcha_validator;
   private $confirm_page;
   private $nlines;
   private $case_insensitive_match;
   ...
   function OnInit()
   {   
      $this->captcha_validator->Init($this->logger,$this->globaldata);
   }
   ...
   function getSessionVarName()
   {
    ...
    function ValidateInstallation(&$app_command_obj)
    {
       ...
       {
            $this->logger->LogInfo("Captcha  ValidateInstallation: error");
            $app_command_obj->addError("GD library functions (required for the CAPTCHA) are not supported on this webserver.\n".
            "Please request your web hosting service to enable GD library with freetype support.");
            
            $app_command_obj->response_sender->appendResponse('captcha','error');
            return false;
       }
        return true;
    }
   ...
   {
      $continue = true;
      if(isset($_GET["sfm_get_captcha"]))
      {  
         $this->Create();

         $session_var = $this->getSessionVarName();
         
         $captcha_pwd = $this->getCaptchaPwd($this->code);
         $this->globaldata->session[$session_var] = $captcha_pwd;
         
         $this->logger->LogInfo("Get new Captcha: newkey:".$_GET["sfm_captcha_k"]." new pwd:".$captcha_pwd);
         
         if(!empty($_GET["sfm_captcha_k"]) && strlen($_GET["sfm_captcha_k"])> 12)
         {
            $instk = $_GET["sfm_captcha_k"];
            
            if(!isset($this->globaldata->session["sfm_captcha_sessions"]))
            {
                $this->globaldata->session["sfm_captcha_sessions"] = array();
            }
            $this->globaldata->session["sfm_captcha_sessions"][$instk] = $captcha_pwd;
         }
            
         $this->ShowCaptcha();
         $continue = false;
      }
      elseif(isset($this->formvars[$this->config->form_submit_variable]) )
      {
      ...
      if(isset($_GET["sfm_validate_captcha"]))
      {
         $this->InitValidator(/*clear_session*/false);

         $error_hash = array();
         if($this->captcha_validator->DoValidate($_GET,$error_hash))
         {
             echo 'success';
         }
         else
   ...
   function DrawLines()
   {
    ...
    function calculateTextBox($font_size, $font_angle, $font_file, $text) 
    {
        ...
        do
        {
            ...
            $count++;
            //PJ 2012 Jan 10
            //Some builds of GD returns bad values for imagettfbbox
            //try again and it gets corrected!
        }while($min_x < 10000 && $count < 10);
   ...
   function DrawChars($code)
   {
      ...
      $tchar = "";
         ...
         for ($i = 0; $i < $N; $i++) 
       {
            ...
            $tchar = sfm_internal_substr($code, $i, 1);
            
            $tbox = $this->calculateTextBox($font_size,$angle,$this->font_file,$tchar);
